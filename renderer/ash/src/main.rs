        ...
        Event::WindowEvent { event, .. } => match event {
            WindowEvent::KeyboardInput { input, .. } => match input.virtual_keycode {
                ...
                Some(VirtualKeyCode::Up) => {
                    ctx.y += MOVEMENT_SPEED * ctx.zoom;
                }
                Some(VirtualKeyCode::Down) => {
                    ctx.y -= MOVEMENT_SPEED * ctx.zoom;
                }
                Some(VirtualKeyCode::Left) => {
                    ctx.x -= MOVEMENT_SPEED * ctx.zoom;
                }
...
pub fn compile_shaders() -> Vec<SpvFile> {
    let mandelbrot_shader = SpvFile {
        name: "mandelbrot_shader".to_string(),
        data: read_spv(&mut std::io::Cursor::new(
            fraq_shader_mandelbrot::SHADER_CODE,
        ))
        .unwrap(),
    };
    vec![mandelbrot_shader]
}
